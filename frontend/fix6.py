...
regex = r'\{sefirot\.map\(\(node\) => \{\s*const item = volumeMap\[node\.id\];\s*const activityCount = item\?\.actividades_total \?\? 0;\s*const scale = activityCount / maxActivityCount;\s*const size = 52 \+ scale \* 38;\s*return \(\s*<div\s*key=\{node\.id\}\s*className=\{bsolute rounded\-full flex flex\-col items\-center\s*justify\-center text\-center border border\-white/30\s*shadow\-\[0_0_20px_rgba\(0,0,0,0\.35\)\] \$\{node\.colorClass\}\}'

newReplacement = '''{sefirot.map((node) => {
...
new_text, count = re.subn(regex, newReplacement, text)
