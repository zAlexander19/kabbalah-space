"""Contenido rico (esencia / palabras clave / qué observar) por sefirá.

Semilla "viva" usada por el startup de la app para rellenar
idempotentemente cualquier sefirá con `esencia` NULL (típicamente una DB
fresca, donde las filas de sefirot las crea el propio startup DESPUÉS de
que la migración `38a8a5cc73c2_sefirot_contenido_rico` ya corrió).

Transcripción VERBATIM del dict `SEED` de esa migración — la migración es
una foto congelada del momento en que se escribió (no se debe tocar
retroactivamente), y esta constante es la copia "viva" que el startup usa
para backfillear DBs nuevas. Duplicación intencional, aprobada por
producto: ver spec de la tarea "contenido de sefirot".
"""

SEFIROT_CONTENIDO_SEED: dict[str, dict] = {
    "keter": {
